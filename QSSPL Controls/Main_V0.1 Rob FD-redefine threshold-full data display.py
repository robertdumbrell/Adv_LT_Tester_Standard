...
class LightPulse():
    ...
    def Define(self):
        # Send a slight positive voltage to ensure the LED is switched off
        V_before =  np.zeros((int(self.OutputSamples*self.Offset_Before/1000)))
        V_after =  0.02*np.ones((int(self.OutputSamples*self.Offset_After/1000)))
#        LED_off = 0.2*np.ones((int(self.OutputSamples*10./1000)))
        if self.Waveform == 'FrequencyScan':
            ...
            # Add the preceding zeros before doing the adjustment for LED turn on
            V=self.Adjust_For_Threshold(np.concatenate((V_before,V)))

        self.Time = self.t[-1]
        # print self.Time
        Voltage_waveform = np.concatenate((V,V_after))
        ...
        np.savetxt(r'D:\Users\Robert\data.txt', Voltage_waveform)
...
#        return -(self.A)*np.abs(np.sin(pi*t/t[-1]))
        return self.A/2 * np.sin(2*pi*t/t[-1] + pi/2) - self.A/2
